...
for idx in range(1,5):
    with open('resultMerge{}.json'.format(idx), 'r', encoding='utf-8') as f:
        dummy = json.load(f)
    categories = set()
    for dum in dummy:
        if len(dum["foodCategory"]) > 0:
            categories.add(dum["foodCategory"])
categories = list(categories)
for categoryIdx in range(len(categories)):

    print("\"{}\"".format(categories[categoryIdx]), end=",")
    if categoryIdx%10 == 0:
        print(categoryIdx//10)
# 기본 세팅 -> 쓰잘떼기없는 애들
# 한식 -> 한식관련을 추가/ 비한식 -> 비한식 관련을 추가
# 육류 -> 육류를 추가/ 해산물 => 해산물을 추가
#
basicCategory = [
"일식튀김,꼬치","전복요리","급식","스페인음식","방앗간",
...
"양꼬치","감자탕","만년닭강정","베이글","핫도그"
"커피가공,제조","4성급","제사음식",
"달떡볶이","차,커피","심야오뎅","ABC커피","정육식당","호떡",
"일식,초밥뷔페","아름다운커피","불닭","수산물","백반,가정식","고기원칙","그리스음식","샤브샤브",
"스파게티스토리","일석삼조버섯매운탕","햄버거","출장요리","유제품제조",
"해산물뷔페","인도음식","힐링카페","초밥,롤","죽",
"육류,고기요리","다이어트,샐러드","닭","막국수","만두",
"해물,생선요리","바나프레소","전통,민속주점","고기뷔페","돈가스","멕시코,남미음식","푸드트럭","곰탕,설렁탕","아부찌부대찌개","분식",
"순대,순댓국","음식점","가리미김밥","제과,제빵","오리요리","문래돼지불백","건어물","아귀찜,해물찜",
"한정식","2성급","보리밥","베이커리","돼지고기구이","뷔페","시골한우시골돼지",
"태국음식","복어요리","기사식당","우동,소바","밀도","바(BAR)","라면",
"술집","양갈비","와인","닭요리","바닷가재요리",
"소고기구이","피자","양식","떡,한과","유흥주점","요리주점","과자,사탕,초코렛",
"칼국수,만두","이북음식","스테이크,립","밀키트","오니기리","슬라임카페","사철,영양탕","도넛",
"보드카페","초콜릿전문점","수산물가공,제조","신의주부대찌개","게요리",
"덮밥","떡류제조","떡볶이","프랜차이즈본사","생선구이","대게요리","찜닭",
"닭갈비","식료품","전통식품","후렌치후라이","슈","주꾸미요리","프랑스음식",
"쌈밥","추어탕","빵류제조","비빔밥","과자류제조","정육점","고양이카페","두부요리",
"생선회","블루보틀","카페,디저트","주류","101커피컨테이너","갈비탕",
"맥주,호프","딤섬,중식만두","이자카야","PC방","해장국",
"푸드코트","일초닭발","오징어요리","낙지요리","토스트","고레카레",
"중식당","아이스크림","대구형제막창","주류제조","닭볶음탕","아시아음식","매운탕,해물탕","고기한끼",
"신룽푸마라탕","우유,유제품","조개요리","곱창,막창,양","와플","전,빈대떡","패밀리레스토랑",
"스파게티,파스타전문","포장마차","일반휴게소",
"테이크아웃커피","종합분식","한식뷔페","독일음식","101번지남산돈까스","굴요리","차류가공,제조",
"단란주점""안경할머니곱창","주먹밥","샌드위치","JVL부대찌개","치킨,닭강정",
"퓨전음식","닭발","채식,샐러드뷔페","스마일찹쌀꽈배기","도시락,조리식품제조"
]

koreanFood = [
    "전복요리","급식","방앗간","고고즉석떡볶이","반찬가게","장금수부대찌개","국수","향토음식",
...
notKoreanFood = [
    "일식튀김,꼬치","스페인음식","퓨전음식","샌드위치","독일음식","스파게티,파스타전문","패밀리레스토랑","와플","신룽푸마라탕",
...
seaFood = [
    "전복요리","굴요리","조개요리","매운탕,해물탕","오징어요리","낙지요리","생선회","추어탕"
